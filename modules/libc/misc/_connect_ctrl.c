	...
	if((fd = ConnectAttach(entry->nd, entry->pid, entry->chid, ctrl->base, _NTO_COF_CLOEXEC)) != -1) {
		...
		*/
		if((ctrl->status & (_IO_CONNECT_RET_FLAG | _IO_CONNECT_RET_TYPE_MASK)) ==
			...
			if(reply->nentries * sizeof(*entry) + reply->path_len >= msg->reply_max) {
				errno = ENAMETOOLONG;
				return -1;
			}
			...
			if(reply->nentries) {
				...
				if (ctrl->flags & FLAG_REUSE_BUFFER) {
					...
					if (!(entry = CTRL_ALLOC(ctrl->flags, num * sizeof(*entry)))) {
				...
				for(; cnt > 0; entry++, cnt--) {
					...
					*/
					...
					if ((msg->file_type == _FTYPE_ANY) || 
					    (entry->file_type == _FTYPE_ALL) || (msg->file_type == entry->file_type)) {
						int								pad_len;
						int								save_errno;
						int								marker;
