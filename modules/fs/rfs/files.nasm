...
NumOfFCBs	RESB	1				; Maximum number of FCBs
FCBstart	RESD	1				; Begin address of FCBs
SemFile		RESD	1				; Semaphore
...
proc RFS_CreateFile
%define	.fslp		ebp-4
%define	.name		ebp-4-DIRENTRYSIZE		; Name buffer

		prologue DIRENTRYSIZE+4
		...
		jnc	near .OldFile			; Yes, go truncate it
		...
		mov	[esi+tFilePage.IAttr+tCFS_IndexAttrs.LWtime],eax ; Fill in time
		mov	word [esi+tFilePage.Sig],FPSignature	; and signature
		mov	edx,[.fslp]
...
%define	.name		ebp-4-DIRENTRYSIZE		; Name buffer
		...
		and	dword [edi+tFCB.Bytes],BLOCKSIZE-1
		shr	eax,BLOCKSHIFT
		mov	[edi+tFCB.Pages],eax
		mov	[edi+tFCB.Device],edx		; Device to FCB
		mov	[edi+tFCB.Page],ebx		; File page to FCB
		mov	dword [edi+tFCB.PosBytes],0	; Position = start of file
		mov	dword [edi+tFCB.PosPages],0
		mov	dword [edi+tFCB.CurrPage],0	; Current page not loaded
...
proc RFS_CloseFile
		...
		mov	dword [edi+tFCB.Page],0		; Release FCB
...
proc RFS_RenameFile
%define	.fslp		ebp-4				; FSLP
%define	.newname	ebp-4-DIRENTRYSIZE		; New name
%define	.oldname	ebp-4-DIRENTRYSIZE*2		; Old name

		prologue DIRENTRYSIZE*2+4		
		mpush	ebx,ecx,edx,esi,edi
		...
		mov	byte [.newname+tDirEntry.Flags],0	; Set params
		mov	dword [.newname+tDirEntry.More],0	; of new entry
		...
		push	dword 0				; Delete old file name
		lea	edi,[.oldname]
		push	edi
		call	RFS_DeleteFileName
		jc	short .Error
...
.DoLoad:	mov	[edi+tFCB.PosBytes],ecx		; Set new position
		and	dword [edi+tFCB.PosBytes],BLOCKSIZE-1
		shr	ecx,BLOCKSHIFT
		mov	[edi+tFCB.PosPages],ecx
		mov	dword [edi+tFCB.CurrPage],0
		mov	eax,ecx
		clc
.Exit:		pop	ecx
		ret
...
proc RFS_ClearFCBs
		push	ecx
		movzx	ecx,byte [NumOfFCBs]		; Number of FCBs
		mov	edi,[FCBstart]
.Loop:		mov	dword [edi+tFCB.Page],0		; Set page to 0
		add	edi,tFCB_size			; Next FCB
...
.Singles:	mov	eax,[esi+4*ecx+tFilePage.Singles] ; Current entry
		...
		jz	short .SNoDeall
		mov	dword [esi+4*ecx+tFilePage.Singles],0	; Else mark unallocated
...
.Doubles:	mpush	ebx,ecx			; Save file page
		...
		mov	dword [esi+4*ecx+tFilePage.Doubles],0	; Get entry
...
proc RFS_GetPosBuffer
%define	.writing	ebp+8			; True if should dirty buffer
...
.OK:		or	ecx,ecx				; See if any to get
		...
		push	dword FALSE			; Else reading again
...
proc RFS_WriteShort
%define	.fslp		ebp-4
%define	.ueof		ebp-8				; Extending EOF if true
...
.Alloc:		mpush	edx,esi				; Allocate the page
		mov	dl,[.fslp]
		call	RFS_ExtendFile
		mpop	esi,edx
